Hafta 2: İşlemci Mimarisi, Bellek Hiyerarşisi ve Veri Odaklı Tasarım Notları
...
// Örnek 2.5: Kötü Mekansal Yerellik (Column-Major Access)
for (i = 0; i < 1000; i++)
    for (j = 0; j < 1000; j++)
    ...
    for (jj = 0; jj < n; jj += B)
        for (i = ii; i < min(ii + B, n); i++)
            for (j = jj; j < min(jj + B, n); j++)
                // Blok bazlı işlem yaparak veriyi cache'de tut
...
Vurgu Kutusu: Bant Genişliği ve Trade-off (Örnek 2.9) Multithreading, gecikmeyi gizlese de toplam bant genişliği ihtiyacını artırır. Tek bir thread %90 hit oranıyla 400 MB/s bant genişliği talep ederken; 32 thread kullanıldığında her birinin cache payı azalacağından (örn. %25 hit oranı), toplam ihtiyaç 3 GB/s seviyesine çıkabilir.
...
* Blok boyutu 1 kelime ise: 100ns'de 1 kelime çekilir \rightarrow 10 MFLOPS.
* Blok boyutu 4 kelime (cache line) ise: 100ns'de çekilen 4 kelime sonraki 4 operasyonun verisini sağlar. Bu durum, mekansal yerellik sayesinde gecikmenin 4 operasyona amortize edilmesini sağlar ve performansı 40 MFLOPS seviyesine çıkarır.

7. Paralel Platformlar ve Kontrol Yapıları

* SIMD (Single Instruction, Multiple Data): Tek bir komutun birden fazla veri üzerinde (örn. vektör işlemcileri) icrasıdır. Örnek 2.11: SIMD mimarisinde if-else blokları performansı düşürür; çünkü 'if' bloğunu çalıştıran işlemciler varken 'else' kısmındakiler atıl beklemek zorundadır (masking).
...
* Ortalama Paralellik Derecesi (Avg Concurrency): Avg\ Concurrency = \frac{W}{L}
