...
Burada:
- $S$: Hızlanma (Speedup) oranını,
- $T_{\mathrm{serial}}$: Problemin tek bir işlemciyle (seri) çözülme süresini,
- $T_{\mathrm{parallel}}$: Problemin birden çok işlemciyle (paralel) çözülme süresini ifade eder.
...
Burada:
- $E$: Verimlilik (Efficiency), yani her bir işlemcinin ne kadar etkin kullanıldığını,
- $S$: Elde edilen hızlanma oranını,
- $p$: Kullanılan işlemci sayısını,
- $T_{\mathrm{serial}}$: Seri çalışma süresini,
- $T_{\mathrm{parallel}}$: Paralel çalışma süresini gösterir.
...
Burada:
- $S(p)$: $p$ adet işlemciyle elde edilen hızlanmayı,
- $r$: Programın toplam çalışma süresinin ne kadarının seri (paralelleştirilemeyen) olduğunu belirten oranı (örn: 0.1, %10'luk kısım seri demektir),
- $p$: Kullanılan işlemci sayısını ifade eder.
...
Burada:
- $S_{\max}$: Teorik olarak ulaşılabilecek maksimum hızlanma oranını,
- $r$: Programın seri olan kısmının oranını gösterir.
...
Burada:
- $S(p)$: Problem boyutuyla ölçeklenmiş hızlanmayı,
- $p$: Kullanılan işlemci sayısını,
- $r$: Programın orijinal (küçük) problem boyutundaki seri kalması gereken kısmının oranını belirtir.
...
Burada:
- $A(i), B(i), C(i)$: Bellekteki büyük veri dizilerini (vektörleri),
- $s$: Skaler (tek) bir sayıyı,
- $i$: Dizinin o anki elemanının indeksini temsil eder.
